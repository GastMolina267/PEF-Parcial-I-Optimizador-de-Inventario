...
**Commit analizado:** `faf6605` · **Generado:** 2026-09-08 13:41 UTC
...
## Estado respecto a `faf6605`

El push que disparó esta corrida es el merge de `#20` (`feat/botones-funcionales`: `e1aaa30` + merge `b77191a`). **Solo tocó** `docs/presentation/app.js` y `docs/presentation/styles.css`: delegación global de clics en pestañas del cuaderno oral y `pointer-events` para que los botones reciban el click sobre el libro 3D.

No hay cambios en `src/`, `benchmarks/`, tests ni `docs/mediciones/`. Los tottime / % Time / speedup / picos de memoria **no se re-midieron**. Las pestañas de la oral ahora pueden mostrar el gráfico de speedup; eso no altera el motor.

Cobertura que sigue vigente desde `7c6af3d` / `a393b1b` (este merge no la cambia):
...
- `src/ui/app.py` / `main.py`: `multiprocessing.freeze_support()` evita que el `.exe` de PyInstaller (Windows, `spawn`) relance la GUI al crear workers.
- `src/motor/motor_inventario.py`: si el caller no pasa `concurrente`, el default sigue siendo `es_optimizado and len(lote) >= 50`.
- `src/ui/pantallas/comparacion.py`: sigue invocando `procesar_pedidos_concurrente` en cada clic a propósito (mide IPC).
...
- **Alternativa:** Selector automático: heap solo si N > 50 o k/N < 0.1; pool de procesos solo si el trabajo por pedido no es un lookup O(1). Documentar el umbral real (hoy el pool pierde hasta `grande.json` en la tabla aislada) en la oral. En Pedidos el pool ya es opt-in; falta el mismo criterio para Top-N, para el default del motor y para Comparativa (sigue midiendo IPC siempre). `faf6605` no cambia esos umbrales: solo habilita las pestañas de la oral para mostrar el gráfico ya medido.
