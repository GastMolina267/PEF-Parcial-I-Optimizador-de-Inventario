...
**Commit analizado:** `7c6af3d` · **Generado:** 2026-09-08 01:48 UTC
...
## Estado respecto a `7c6af3d`

El push que disparó esta corrida **no tocó** `docs/mediciones/`. Los tottime / % Time / speedup siguen siendo los mismos informes. Sí cambió el motor/UI de preparación:

- `src/ui/pantallas/pedidos.py`: el switch de ProcessPool arranca en `False` y **ya no se enciende** al pasar a Optimizado. Optimizado usa hash O(1) en secuencial salvo que el operador active IPC.
...
Los perfiles de cProfile (CreateProcess 0.364 s) son **anteriores** al pool persistente. Hay que volver a correr `perfilar_cprofile` / `comparar` antes de dar por cerrado el IPC.
